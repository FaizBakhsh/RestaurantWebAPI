...
INSERT INTO `modifier` VALUES (1,73,NULL,1024001,'Modifier name....',200.98,12,'2023-04-08 15:15:10',0,'2023-04-08 15:15:10',0,_binary ''),(2,65,NULL,1024001,'TESTING',987.4,1,'2023-04-08 15:17:14',0,'2023-04-08 15:17:14',0,_binary '\0'),(3,100,1024002,1024001,'SAUCE',0,43,'2023-06-10 17:45:46',0,'2023-06-10 17:45:46',0,_binary '');
...
if DBoperation = 'Insert'
then
           insert into modifier (Id,modifierName,rawItemId,kDSId,kDSId2,price,qty,
           CreatedOn, CreatedById,
           ModifiedOn, ModifiedById, IsActive ) 
           values (id,modifierName,rawItemId,kDSId,kDSId2,price,qty,
           createdOn, createdById, modifiedOn,
           modifiedById, isActive );
 end if;   
/*update*/
...
/*!50001 VIEW `vw_modifier` AS select `modifier`.`Id` AS `Id`,`modifier`.`ModifierName` AS `ModifierName`,`modifier`.`Price` AS `Price`,`modifier`.`RawItemId` AS `RawItemId`,`modifier`.`Qty` AS `Qty`,`item`.`ItemName` AS `RawItem`,`modifier`.`KDSId` AS `KDSId`,`kds`.`Name` AS `KDS`,`modifier`.`KDSId2` AS `KDSId2`,`kds`.`Name` AS `KDS2`,`modifier`.`CreatedOn` AS `CreatedOn`,`modifier`.`CreatedById` AS `CreatedById`,`modifier`.`ModifiedOn` AS `ModifiedOn`,`modifier`.`ModifiedById` AS `ModifiedById`,`modifier`.`IsActive` AS `IsActive` from (((`modifier` join `enumline` `kds` on((`kds`.`Id` = `modifier`.`KDSId`))) join `enumline` `2kds` on((`2kds`.`Id` = `modifier`.`KDSId`))) join `item` on((`item`.`Id` = `modifier`.`RawItemId`))) */;
...
-- Dump completed on 2023-06-12 12:14:33
