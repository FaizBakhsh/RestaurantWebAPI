...
import org.example.entity.LogEvent;
...
public class LogEventRptController {

    public LogEventRptController() {
    }
    ...
    public ResponseEntity<byte[]> generateReport(@RequestBody Map<String, Object> request, @PathVariable("period") String period) {
        try {
            ...
            List<LogEvent> sales = mapper.convertValue(request.get("ReportData"), new TypeReference<List<LogEvent>>() {
            });
            CompanyInfo companyProfile = request.containsKey("CompanyProfile")
                    ...
                    : null;
            File file;
            file = ResourceUtils.getFile("classpath:LogEventRpt.jrxml");
