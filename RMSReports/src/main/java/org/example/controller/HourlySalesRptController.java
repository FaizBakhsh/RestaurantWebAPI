...
@CrossOrigin(origins = "*")
@RestController
@RequestMapping("/api/HourlySales")
public class HourlySalesRptController {

    public HourlySalesRptController() {
    }

    @PostMapping("/Report/{date}")
    public ResponseEntity<byte[]> generateReport(@RequestBody Map<String, Object> request, @PathVariable("date") String period) {
        try {
            System.out.print(period);
            ObjectMapper mapper = new ObjectMapper();
            List<HourlySale> sales = mapper.convertValue(request.get("ReportData"), new TypeReference<List<HourlySale>>() {
            });
            ...
            CompanyInfo companyProfile = request.containsKey("CompanyProfile")
                    ...
                    : null;
            File file;
            file = ResourceUtils.getFile("classpath:HourlySalesRpt.jrxml");
