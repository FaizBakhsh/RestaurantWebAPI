...
@CrossOrigin(origins = "*")
@RestController
@RequestMapping("/api/DiscountSale")
public class DiscountSaleRptController {

    public DiscountSaleRptController() {
    }
    ...
    public ResponseEntity<byte[]> generateReport(@RequestBody Map<String, Object> request, @PathVariable("period") String period) {
        try {
            ...
            List<DiscountSale> sales = mapper.convertValue(request.get("ReportData"), new TypeReference<List<DiscountSale>>() {
            });
            CompanyInfo companyProfile = request.containsKey("CompanyProfile")
                    ...
                    : null;
            File file;
            file = ResourceUtils.getFile("classpath:DiscountSaleRpt.jrxml");
