...
@CrossOrigin(origins = "*")
@RestController
@RequestMapping("/api/RawItem")
public class RawItemListController {

    public RawItemListController() {
    }
    ...
    public ResponseEntity<byte[]> generateReport(@RequestBody Map<String, Object> request) {
        try {
            ...
            List<RawItem> sales = mapper.convertValue(request.get("ReportData"), new TypeReference<List<RawItem>>() {
            });
            ...
            CompanyInfo companyProfile = request.containsKey("CompanyProfile")
                    ...
                    : null;
            File file;
            file = ResourceUtils.getFile("classpath:RawItemList.jrxml");
