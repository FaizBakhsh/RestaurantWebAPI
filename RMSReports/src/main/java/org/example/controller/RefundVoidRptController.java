...
import org.example.entity.RefundVoidSale;
...
@CrossOrigin(origins = "*")
@RestController
@RequestMapping("/api/RefundVoid")
public class RefundVoidRptController {

    public RefundVoidRptController() {
    }
    ...
    public ResponseEntity<byte[]> generateReport(@RequestBody Map<String, Object> request, @PathVariable("period") String period) {
        try {
            ...
            List<RefundVoidSale> sales = mapper.convertValue(request.get("ReportData"), new TypeReference<List<RefundVoidSale>>() {
            });
            ...
            CompanyInfo companyProfile = request.containsKey("CompanyProfile")
                    ...
                    : null;
            File file;
            file = ResourceUtils.getFile("classpath:RefundVoidRpt.jrxml");
